...
class PHPJasperXMLSubReport {
  ...
  public function __construct($lang = "en", $pdflib = "FPDF", $xoffset = 0) {
    $this->lang = $lang;
    error_reporting ( 1 );
    $this->pdflib = $pdflib;
    ...
    switch ($data->textFieldExpression) {
      ...
      case '$V{' . $this->gnam . '_COUNT}' :
        ...
        $this->pointer [] = array (
            ...
            "width" => $data->reportElement ["width"],
            "height" => $height,
            "txt" => &$this->group_count ["$this->gnam"],
            "border" => $border,
            "align" => $align,
            "fill" => $fill,
...
else // reset type='group'
{
      ...
      foreach ( $this->arraypageFooter as $out ) {
        switch ($out ["hidden_type"]) {
          case "field" :
            ...
            if ($this->pdf->getY () > $this->SubReportCheckPoint) {
              $this->SubReportCheckPoint = $this->pdf->getY ();
            }
  ...
  public function lastPageFooter($checkpoint) {
    if (isset ( $checkpoint )) {
      ...
      $Y = $this->arrayPageSetting ["pageHeight"] - $this->arraylastPageFooter [0] ["height"] - $this->arrayPageSetting ["bottomMargin"];
    }
  ...
  public function display($arraydata, $y_axis = 0, $fielddata = false) {
    ...
    if ($arraydata ["type"] == "SetFont") {
      ...
      if (file_exists ( $path ) || left ( $path, 4 ) == 'http') {
        ...
        $this->pdf->Image ( '@' . $imgdata, $arraydata ["x"] + $this->arrayPageSetting ["leftMargin"], $arraydata ["y"] + $y_axis, $arraydata ["width"], $arraydata ["height"] ); // ,$imgtype,$arraydata["link"]);
